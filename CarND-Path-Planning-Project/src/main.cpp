    ...
    private:
        ...
        // zero lane occupied by the car so that the car
			...
			void updateCost(double car_pos, int car_lane, double car_vel,
											...
											double gap_front, double gap_rear, double gap_front_next, double max_speed, double cost,
											bool cost_ready, bool change_in_progress) {
          ...
          // j[1] is the data JSON object
                ...
                // car is in my lane
								...
								cost.updateCost(
								  ...
								  // another car position along the road, lane and speed
									...
									// this is to check if exact postion of our car
									// is more than 1 lane width from the destination lane
									(abs(car_d/4 - lane)) > 1);
						}
            //cout << "sensor_fusion " << sensor_fusion.size() << endl;
          	...
          	//vector<double> next_y_vals;
						...
						// Create a list of widly spaced (x,y) waypoints, evenly spaced at 30m
						// Later we will interpolate these waypoints with the spline and fill it in with more points that control spline
						// Anchor points
						vector<double> ptsx;
						vector<double> ptsy;

						// reference x, y, yaw_states
						// either we will reference the starting points as where the car is or at previous path's end point
						double ref_x = car_x;
						double ref_y = car_y;
            ...
            double target_x = 30.0; // that's our horizon
            double target_y = s(target_x); // just ask spline what y is for the given x
            double target_dist = sqrt(target_x * target_x + target_y * target_y); 
            double x_add_on = 0;
            
						// fill up the rest of our path planner after filling it with previous points
						// note, we will always output 50 points
            for( int i = 1; i < 50 - previous_path_x.size(); i++ ) {
							// dividing it by 2.24 because it was miles per hour and needs to be meters per second
              ...
              double y_ref = y_point;
							...
							// rotate back to normal (global coordinates) after rotating it (local coordinates) ealier
